          ...
          setTimeout(() => {
            setNumbers(prevNumbers =>
              prevNumbers.map((num, index) => {
                const currentValue = Math.round((targetNumbers[index] * i) / steps);
...
{/* Signature Image Section */}
      ...
      {/* Hero Section */}
      <div ref={heroRef} className="flex items-center justify-center p-4 w-full overflow-hidden">
        ...
        <div className="flex animate-scroll-left">
          ...
          <div className="flex items-center py-4 min-w-max">
            {[
              ...
              <Icon key={index} className="text-gray-400 w-8 h-8 mx-6" />
            ))}
          </div>
          {/* Duplicate set */}
          <div className="flex items-center py-4 min-w-max">
            {[
              ...
              <Icon key={`dup-${index}`} className="text-gray-400 w-8 h-8 mx-6" />
            ))}
          </div>
        </div>
      </div>
                    ...
                    <span key={i} className="flex items-center">
                      {"Generative AI * Custom Coding * Research * DevOps".split('*').map((text, index) => (
                        <span key={index} className="flex items-center">
                          <span>{text}</span>
                          <span className="text-orange-300 mx-3 transform rotate-45 font-regular">✱</span>
                        </span>
                      ))}
                    </span>
                  ))}
       ...
       { number: "50K", label: "Photography Collection" }
        ...
        >
          <div className="text-7xl font-extralight mb-2 tracking-wide text-white">
            <span style={{ fontFamily: "Arial, sans-serif", fontWeight: 100 }}>
            {numbers[index]?.toString().replace(/[KM+]/g, '')}
            </span>
            <span style={{ fontFamily: "'Cormorant Garamond', serif" }}>
              {numbers[index]?.toString().match(/[KM+]/g)}
            </span>
            <span className="text-orange-200/90" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
              +
            </span>
          </div>
